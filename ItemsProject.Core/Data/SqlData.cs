...
namespace ItemsProject.Core.Data
{
    public class SqlData : IDatabaseData
    {
        ...
        public async Task DefaultHotwheelsDbPopulation()
        {
            ...
            if (isDbPopulated == 0)
            {
                ...
                for (int year = availableYears.Last(); year >= 1980; year--)
                {
                    ...
                    foreach (HotWheelsModel car in hotWheelsModels)
                    {
                        ...
                        await _db.SaveData("spHotwheelsCars_InsertCar",
                                     new
                                     {
            ...
            else
            {
                ...
                for (int year = currentYear; year <= availableYears.Last(); year++)
                {
                    ...
                    foreach (HotWheelsModel car in hotWheelsModels)
                    {
                        await _db.SaveData("spHotwheelsCars_UpdateCurrentYear",
                                     new
                                     {
                                         modelName = car.ModelName,
                                         seriesName = car.SeriesName,
        ...
        {
            List<int> output = await _db.LoadData<int, dynamic>("spAppSettings_IsDbPopulated", new { }, connectionStringName, true);
            return output.First();
        }

        public async Task SetDbToPopulated()
        {
            await _db.SaveData("spAppSettings_SetDbToPopulated", new { }, connectionStringName, true);
        }

        public async Task<List<ItemModel>> GetAllItems()
        {
            List<ItemModel> output = await _db.LoadData<ItemModel, dynamic>("spItems_GetAll", new { }, connectionStringName, true);
            return output;
        }

        public async Task<List<FolderModel>> GetAllFolders()
        {
            List<FolderModel> output = await _db.LoadData<FolderModel, dynamic>("spFolders_GetAll", new { }, connectionStringName, true);
            return output;
        }

        public async Task<List<ItemModel>> GetItemsByFolderId(int folderId)
        {
            List<ItemModel> output = await _db.LoadData<ItemModel, dynamic>("spItems_GetByFolderId", new { folderId }, connectionStringName, true);
            return output;
        }
        ...
        public async Task<FolderModel> CreateNewFolder(string folderName)
        {
            ...
            List<FolderModel> output = await _db.LoadData<FolderModel, dynamic>("spFolders_GetLast", new { }, connectionStringName, true);
            return output.First();
        }

        public async Task<ItemModel> GetItemById(int itemId)
        {
            List<ItemModel> output = await _db.LoadData<ItemModel, dynamic>("spItems_GetById", new { itemId }, connectionStringName, true);
            return output.First();
        }

        public async Task DeleteItem(int itemId, int folderId)
        {
            await _db.SaveData("spItems_Remove", new { itemId, folderId }, connectionStringName, true);
        }

        public async Task<FolderModel> GetFolderById(int folderId)
        {
            List<FolderModel> output = await _db.LoadData<FolderModel, dynamic>("spFolders_GetById", new { folderId }, connectionStringName, true);
            return output.First();
        }
        ...
        {
            await _db.SaveData("spFolders_EditName", new { folderName, folderId }, connectionStringName, true);
        }

        public async Task EditItem(int itemId, string newName, string newReleaseDate, string newCollectionName)
        {
            await _db.SaveData("spItems_EditItem", new { itemId, newName, newReleaseDate, newCollectionName }, connectionStringName, true);
        }

        public async Task<List<int>> GetAllFolderIdsForItem(int selectedItemID)
        {
            List<int> output = await _db.LoadData<int, dynamic>("spFolders_GetAllFolderIdsForItem", new { selectedItemID }, connectionStringName, true);
            return output;
        }

        public async Task AddItemToFolder(int selectedItemId, int selectedFolderId)
        {
            await _db.SaveData("spItems_AddToFolder", new { selectedItemId, selectedFolderId }, connectionStringName, true);
        }

        public async Task DeleteAllItemsFromFolder(int itemId)
        {
            await _db.SaveData("spItems_RemoveAllFromFolder", new { itemId }, connectionStringName, true);
        }

        public async Task<List<HotWheelsModel>> SearchHotWheels(string searchhwText)
        {
            List<HotWheelsModel> searchResult = await _db.LoadData<HotWheelsModel, dynamic>("spHotwheelsCars_FindCarByText", new { searchhwText }, connectionStringName, true);
            return searchResult;
        }
        ...
        {
            List<ItemModel> output = await _db.LoadData<ItemModel, dynamic>("spItems_GetAllNonCustom", new {}, connectionStringName, true);
            return output;
        }

        public async Task<int> GetAllQuantities()
        {
            List<int> output = await _db.LoadData<int, dynamic>("spItems_GetAllQuantities", new { }, connectionStringName, true);
            return output.FirstOrDefault();
        }

        public async Task<ItemModel> RemoveOneQuantity(ItemModel? itemModel)
        {
            await _db.SaveData("spItems_RemoveOneQuantity", new { itemId = itemModel.Id }, connectionStringName, true);
            List<ItemModel> output = await _db.LoadData<ItemModel, dynamic>("spItems_GetUnique",
        ...
        {
            List<FolderModel> output = await _db.LoadData<FolderModel, dynamic>("spFolders_GetDefault", new {}, connectionStringName, true);
            return output.First();
        }
        ...
        public async Task<List<ItemModel>> GetLatestCars()
        {
            ...
            List<ItemModel> output = await _db.LoadData<ItemModel, dynamic>("spItems_GetLatest", new { currentYear, lastYearHw }, connectionStringName, true);
